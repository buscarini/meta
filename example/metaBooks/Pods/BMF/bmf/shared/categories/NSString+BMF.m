...
static BMFHTMLEscapeMap gAsciiHTMLEscapeMap[] = {
...
static BMFHTMLEscapeMap gUnicodeHTMLEscapeMap[] = {
...
static int BMFEscapeMapCompare(const void *ucharVoid, const void *mapVoid) {
	const unichar *uchar = (const unichar*)ucharVoid;
	const BMFHTMLEscapeMap *map = (const BMFHTMLEscapeMap*)mapVoid;
	int val;
	if (*uchar > map->uchar) {
		val = 1;
...
- (NSString *) BMF_stringByEscapingHTMLUsingTable:(BMFHTMLEscapeMap*)table
	...
	for (NSUInteger i = 0; i < length; ++i) {
		BMFHTMLEscapeMap *val = bsearch(&buffer[i], table,
									 size / sizeof(BMFHTMLEscapeMap),
									 sizeof(BMFHTMLEscapeMap), BMFEscapeMapCompare);
...
- (NSString *) BMF_stringByEscapingForHTML {
	return [self BMF_stringByEscapingHTMLUsingTable:gUnicodeHTMLEscapeMap
											 ofSize:sizeof(gUnicodeHTMLEscapeMap)
									escapingUnicode:NO];
}

- (NSString *) BMF_stringByEscapingForASCIIHTML {
	return [self BMF_stringByEscapingHTMLUsingTable:gAsciiHTMLEscapeMap
	...
	do {
		...
		if (length > 3 && length < 11) {
			if ([escapeString characterAtIndex:1] == '#') {
				...
				// "standard" sequences
				for (unsigned i = 0; i < sizeof(gAsciiHTMLEscapeMap) / sizeof(BMFHTMLEscapeMap); ++i) {
...
- (CGRect) BMF_rectWithFont:(BMFIXFont *) font maxSize:(CGSize) maxSize {
	return [self BMF_rectWithAttributes:@{ NSFontAttributeName : font } maxSize:maxSize];
}
